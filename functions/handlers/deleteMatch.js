...
exports.deleteMatch = onCall(async request => {
  ...
  try {
    const listingId = `matches_${matchId}`;
    ...
    if (listingSnap.exists) {
      ...
      logger.info('deleteMatch: listing deleted', { listingId, matchId });
    }
  } catch (err) {
    logger.warn('deleteMatch: failed to delete publicMatchListings', { matchId, err: String(err) });
  }
